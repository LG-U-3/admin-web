...
public interface MessageReservationRepository extends JpaRepository<MessageReservation, Long> {
  ...
  @Query("SELECT m FROM MessageReservation m " + "JOIN FETCH m.userGroup "
      + "JOIN FETCH m.template " + "WHERE m.id = :id")
  ...
  @Query("""
        update MessageReservation r
         ...
         where r.id = :id
      """)
  void updateStatus(@Param("id") Long id, @Param("statusId") Long statusId);
}
