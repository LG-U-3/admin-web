...
CREATE TABLE message_send_results (
  id               BIGINT NOT NULL AUTO_INCREMENT,
  reserved_send_id BIGINT NULL,             -- 예약 발송 id(FK)
  user_id          BIGINT NULL,             -- 특정 회원 대상 발송이면 FK
  channel_id       VARCHAR(10) NOT NULL,    -- 정의서 그대로(VARCHAR). 예: EMAIL/SMS
  template_id      BIGINT NULL,
  status_id        BIGINT NOT NULL,         -- 정의서 그대로(BIGINT). 기본값은 V2/앱에서 세팅 권장
